...
exports.run = async(client, message, args) => { 
  ...
  .setTitle('GHOST ᴻ Spectre Level System')
   .setAuthor(message.member.user.username, message.author.avatarURL)
  .addField('Member:', user, true)
  .addField('XP:', '**'+kontrol+'**',) 
  .addField('Level:', '**'+kontrol2+'**',) 
  .setFooter('This system is created by vac.')
  .setColor('#d774ff')
  .setTimestamp()
  .setThumbnail(user.avatarURL)
  message.channel.send(seviye).then(a=> a.delete(10000))
